...
function updateHeartsDisplay() {
    ...
    for (let i = 0; i < total; i++) {
        h += `<span class="heart-icon${i >= G.livesLeft ? ' lost' : ''}">❤️</span>`;
    }
    row.innerHTML = h;
}
    ...
    if (G.op === 'add' || G.currentCatKey === 'addition' || q.text?.includes('+')) {
        ...
        if (a <= 5 && b <= 5) {
            ...
            for (let i = 0; i < a; i++) {
                ...
                for (let j = 0; j < b; j++) h += `<div class="visual-dot" style="width:14px;height:14px;background:var(--accent2)"></div>`;
                h += '</div>';
            }
            box.innerHTML = h;
        }
...
function checkAnswer(btn) {
    ...
    if (val === G.correctAnswer) {
        ...
        if (timerActive) {
            ...
            else { document.getElementById('timerBar').classList.remove('danger'); bt.classList.remove('danger'); }
        }
...
function startTrainingMode(op = 'mix') {
    clearGameTimer();
    G.mode = 'classic'; G.op = op; G.score = 0; G.correct = 0; G.wrong = 0;
    G.streak = 0; G.bestStreak = 0; G.currentQ = 0; G.totalQ = 9999;
    G.coinsEarned = 0; G.answered = false; G.ended = false;
    G.isTraining = true; G.livesLeft = 99; G.customTable = null;
    G.hasTimer = false; G.helpersUsed = { skip: false, remove: false };
    G.askedQuestions = [];
...
function startGameWith(mode, op, customTable = null, forceTimer = false) {
    ...
    G.mode = mode; G.op = op; G.score = 0; G.correct = 0; G.wrong = 0;
    G.streak = 0; G.bestStreak = 0; G.currentQ = 0; G.coinsEarned = 0;
    G.answered = false; G.ended = false; G.isTraining = false;
    G.customTable = customTable || null; G.askedQuestions = [];
    ...
    G.livesLeft = lives; G.hasTimer = hasTimer; G.helpersUsed = { skip: false, remove: false };
    ...
    if (hasTimer) {
        document.getElementById('bigTimerWrap').style.display = 'block';
        const bt = document.getElementById('bigTimer'); bt.textContent = G.timeLeft; bt.classList.remove('danger');
        document.getElementById('timerBar').style.width = '100%'; document.getElementById('timerBar').classList.remove('danger');
        if (G.timer) clearInterval(G.timer);
        G.timer = setInterval(() => {
            if (G.ended) { clearInterval(G.timer); G.timer = null; return; }
            if (G.timeLeft <= 0) { clearInterval(G.timer); G.timer = null; if (!G.ended) endGame(); }
...
function useHelper(type) {
    ...
    if (type === 'skip') {
        ...
        if (G.hasTimer && G.maxTime > 0) {
            ...
            const pct = (G.timeLeft / G.maxTime) * 100; document.getElementById('timerBar').style.width = pct + '%';
            const bt = document.getElementById('bigTimer'); if (bt) bt.textContent = G.timeLeft;
            if (G.timeLeft <= 0) { clearGameTimer(); endGame(); return; }
        }
        ...
        st.coins -= 7; G.livesLeft++;
        saveSt(); updateUI(); updateHeartsDisplay(); updateGameCoinsDisplay();
