...
function checkAnswer(btn) {
    ...
    if (val === G.correctAnswer) {
        ...
        if (G.mode === 'competition') {
            G.score += 1; // 1 نقطة لكل إجابة صحيحة
            G.coinsEarned += 0.1;
        } else {
        ...
        if (timerActive) {
            ...
            if (G.timeLeft <= 10) { document.getElementById('timerBar').classList.add('danger'); bt.classList.add('danger'); } else { document.getElementById('timerBar').classList.remove('danger'); bt.classList.remove('danger'); }
        }
        if (G.currentCatKey) {
            if (!st.stats[G.currentCatKey]) st.stats[G.currentCatKey] = { att:0, cor:0, first:0, stars:0, max:0 };
    ...
    setTimeout(() => {
        if (G.ended) return;
        if (!G.isTraining && G.mode !== 'speed' && G.mode !== 'survival' && G.mode !== 'frenzy' && G.mode !== 'competition' && G.currentQ >= G.totalQ) endGame();
        else loadQuestion();
    }, delay);
}
...
function endGame() {
    ...
    if (!G.isTraining) {
        ...
        if (G.mode === 'competition') {
            if (G.score > (st.bestCompScore || 0)) st.bestCompScore = G.score;
        } else if (G.score > st.bestScore) { st.bestScore = G.score; }
        ...
        if (['classic', 'speed', 'survival', 'frenzy', 'competition'].includes(G.mode)) { st.catCounter.correct += G.correct; st.catCounter.total += G.correct + G.wrong; }
        ...
        document.getElementById('resultsSub').textContent = G.mode === 'competition' ?
            `وصلت للسؤال ${G.correct} • ${G.wrong} خطأ • صعوبة متصاعدة` :
