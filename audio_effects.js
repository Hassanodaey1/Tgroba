/* ═══════════════════════════════════════════════════════════════
   HO Math — نظام الصوتيات الاحترافي v3.0
   © 2026 Hassan Odaey
   
   الميزات:
   ① Web Audio API مع مؤثرات صوتية حقيقية (reverb, compressor, EQ)
   ② موسيقى خلفية تكيّفية (تتسارع مع الضغط، تتوقف مع النهاية)
   ③ مؤثرات بصرية-صوتية متزامنة
   ④ نظام اهتزاز ذكي متعدد المستويات
   ⑤ تخزين مؤقت للمقاطع الصوتية (audio cache)
═══════════════════════════════════════════════════════════════ */

/* ═══════════════════════════════════════════════════════════════
   GAME STATE (نُبقيه هنا للتوافق مع باقي الكود)
═══════════════════════════════════════════════════════════════ */
var G = {
    mode: 'classic', op: 'mix', score: 0, correct: 0, wrong: 0, streak: 0,
    bestStreak: 0, currentQ: 0, totalQ: 10, correctAnswer: 0, answered: false,
    timer: null, timeLeft: 0, maxTime: 0, coinsEarned: 0, livesLeft: 3,
    maxLives: 3,
    helpersUsed: { skip: false, remove: false, heart: false }, ended: false, isTraining: false,
    _survivalWrong: 0,
    customTable: null, hasTimer: false, askedQuestions: [], currentExplanation: '',
    currentCatKey: ''
};

function clearGameTimer() {
    if (G.timer) { clearInterval(G.timer); G.timer = null; }
}

/* ═══════════════════════════════════════════════════════════════
   PARTICLES & SPLASH SYMBOLS
═══════════════════════════════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', function () {
    const c = document.getElementById('particles');
    if (!c) return;
    const col = ['#f0b90b', '#7c3aed', '#06b6d4', '#10b981'];
    for (let i = 0; i < 18; i++) {
        const p = document.createElement('div');
        p.className = 'particle';
        p.style.cssText = `left:${Math.random() * 100}%;background:${col[~~(Math.random() * 4)]};width:${2 + Math.random() * 3}px;height:${2 + Math.random() * 3}px;animation-delay:${Math.random() * 9}s;animation-duration:${6 + Math.random() * 7}s;`;
        c.appendChild(p);
    }
    const sym = ['∑', '∏', '√', '∞', 'π', 'Δ', '∫', '∂', '±', '×', '÷', '=', 'α', 'β', 'θ', 'λ', 'μ', 'σ', 'φ', 'ψ', 'Ω', '∈', '∀', '∃', '≅', '≈', '≠', '≤', '≥', '+', '-', '*', '/'];
    let container = document.getElementById('splashSymbols');
    if (!container) return;
    container.innerHTML = '';
    for (let i = 0; i < 55; i++) {
        let span = document.createElement('span');
        span.textContent = sym[Math.floor(Math.random() * sym.length)];
        let size = Math.random() * 2 + 0.8;
        span.style.cssText = `position:absolute;left:${Math.random() * 100}%;top:${Math.random() * 100}%;font-size:${size}em;opacity:${Math.random() * 0.3 + 0.05};transform:rotate(${Math.random() * 360}deg);animation:floatSymbol ${Math.random() * 10 + 8}s infinite alternate ease-in-out;animation-delay:-${Math.random() * 5}s;`;
        span.classList.add('animated-symbol');
        container.appendChild(span);
    }
});

/* ═══════════════════════════════════════════════════════════════
   ① نواة الصوت — Audio Context مع Compressor احترافي
═══════════════════════════════════════════════════════════════ */
var aCtx = null;
var _masterGain = null;
var _compressor = null;
var _reverbNode = null;
var bgInt = null;
var _bgGain = null;
var _bgBpm = 120;          /* BPM الخلفية الحالي — يتغير مع شدة اللعبة */
var _bgAccelerated = false; /* هل المؤقت ضاغط؟ */

function gACtx() {
    if (!aCtx) {
        try {
            aCtx = new (window.AudioContext || window.webkitAudioContext)();
            _setupAudioGraph();
        } catch (e) { console.warn('AudioContext غير متاح:', e); }
    }
    if (aCtx && aCtx.state === 'suspended') {
        aCtx.resume().catch(() => {});
    }
    return aCtx;
}

/* إنشاء الرسم البياني الصوتي: Source → EQ → Compressor → MasterGain → Dest */
function _setupAudioGraph() {
    if (!aCtx) return;
    /* Master Gain */
    _masterGain = aCtx.createGain();
    _masterGain.gain.value = 1.0;
    _masterGain.connect(aCtx.destination);

    /* Dynamic Compressor — يمنع القطع عند تشغيل مؤثرات متعددة */
    _compressor = aCtx.createDynamicsCompressor();
    _compressor.threshold.value = -18;
    _compressor.knee.value = 8;
    _compressor.ratio.value = 4;
    _compressor.attack.value = 0.003;
    _compressor.release.value = 0.12;
    _compressor.connect(_masterGain);

    /* Reverb بسيط (Convolver) للمؤثرات الاحتفالية */
    try {
        _reverbNode = _buildSimpleReverb(aCtx, 0.8);
        _reverbNode.connect(_masterGain);
    } catch(e) {}

    /* Gain الخلفية */
    _bgGain = aCtx.createGain();
    const vol = (typeof st !== 'undefined' && typeof st.bgVolume === 'number' ? st.bgVolume : 60) / 100;
    _bgGain.gain.value = 0.022 * vol;
    _bgGain.connect(_compressor);
}

/* بناء reverb بسيط عبر impulse response اصطناعي */
function _buildSimpleReverb(ctx, durationSec) {
    const conv = ctx.createConvolver();
    const sampleRate = ctx.sampleRate;
    const length = Math.floor(sampleRate * durationSec);
    const buf = ctx.createBuffer(2, length, sampleRate);
    for (let ch = 0; ch < 2; ch++) {
        const data = buf.getChannelData(ch);
        for (let i = 0; i < length; i++) {
            data[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / length, 2.5);
        }
    }
    conv.buffer = buf;
    /* Reverb Gain — خفيف جداً، فقط للإضافة لا السيطرة */
    const rvGain = ctx.createGain();
    rvGain.gain.value = 0.08;
    conv.connect(rvGain);
    rvGain.connect(ctx.destination);
    return conv;
}

/* ═══════════════════════════════════════════════════════════════
   ② مولّد النغمات الأساسي — مع envelope احترافية
═══════════════════════════════════════════════════════════════ */

/**
 * تشغيل نغمة بسيطة
 * @param {number} f - التردد
 * @param {string} t - نوع الموجة (sine/square/sawtooth/triangle)
 * @param {number} d - المدة بالثواني
 * @param {number} v - مستوى الصوت (0-1) قبل التطبيق
 * @param {number} delay - تأخير بالثواني
 * @param {boolean} useReverb - هل نستخدم الـ reverb؟
 */
function tone(f, t = 'sine', d = 0.25, v = 0.12, delay = 0, useReverb = false) {
    if (!st || !st.soundOn) return;
    const ctx = gACtx();
    if (!ctx) return;
    const vol = (typeof st.soundVolume === 'number' ? st.soundVolume : 80) / 100;

    const o = ctx.createOscillator();
    const g = ctx.createGain();

    /* EQ خفيف لتحسين الصوت — High-Shelf لإضافة حدة */
    const eq = ctx.createBiquadFilter();
    eq.type = 'highshelf';
    eq.frequency.value = 3000;
    eq.gain.value = 3;

    o.connect(eq);
    eq.connect(g);

    /* توجيه للـ reverb إذا طُلب */
    if (useReverb && _reverbNode) {
        g.connect(_reverbNode);
    }
    g.connect(_compressor || ctx.destination);

    o.type = t;
    o.frequency.setValueAtTime(f, ctx.currentTime + delay);

    const ts = ctx.currentTime + delay;
    /* Attack سريع → Sustain → Release */
    g.gain.setValueAtTime(0, ts);
    g.gain.linearRampToValueAtTime(v * vol, ts + 0.008);       /* attack 8ms */
    g.gain.setValueAtTime(v * vol * 0.85, ts + d * 0.6);       /* sustain */
    g.gain.exponentialRampToValueAtTime(0.0001, ts + d);       /* release */

    o.start(ts);
    o.stop(ts + d + 0.01);
}

/* نغمة مع glide (pitch slide) */
function toneGlide(fStart, fEnd, t = 'sine', d = 0.3, v = 0.12, delay = 0) {
    if (!st || !st.soundOn) return;
    const ctx = gACtx();
    if (!ctx) return;
    const vol = (typeof st.soundVolume === 'number' ? st.soundVolume : 80) / 100;
    const o = ctx.createOscillator();
    const g = ctx.createGain();
    o.connect(g);
    g.connect(_compressor || ctx.destination);
    o.type = t;
    const ts = ctx.currentTime + delay;
    o.frequency.setValueAtTime(fStart, ts);
    o.frequency.exponentialRampToValueAtTime(fEnd, ts + d);
    g.gain.setValueAtTime(0, ts);
    g.gain.linearRampToValueAtTime(v * vol, ts + 0.01);
    g.gain.exponentialRampToValueAtTime(0.0001, ts + d);
    o.start(ts);
    o.stop(ts + d + 0.01);
}

/* ═══════════════════════════════════════════════════════════════
   ③ مكتبة المؤثرات الصوتية — كل مؤثر مصمم بعناية
═══════════════════════════════════════════════════════════════ */

function playSound(type) {
    if (!st || !st.soundOn) return;
    const ctx = gACtx();
    if (!ctx) return;

    switch (type) {

        /* ✅ صحيح — نغمة صاعدة منعشة مع overtone */
        case 'correct': {
            tone(523, 'sine', 0.10, 0.10);                          /* C5 */
            tone(659, 'sine', 0.12, 0.09, 0.07);                    /* E5 */
            tone(784, 'sine', 0.14, 0.08, 0.14);                    /* G5 */
            /* overtone خفي للعمق */
            tone(1047, 'triangle', 0.10, 0.04, 0.14);               /* C6 */
            break;
        }

        /* ❌ خطأ — نغمة هابطة مع distortion خفي */
        case 'wrong': {
            tone(300, 'sawtooth', 0.08, 0.09);
            toneGlide(220, 140, 'sawtooth', 0.22, 0.07, 0.06);
            /* bass thump */
            tone(80, 'sine', 0.18, 0.06, 0.0);
            break;
        }

        /* 🏆 ارتقاء مستوى — موسيقى احتفالية كاملة مع reverb */
        case 'levelup': {
            const melody = [
                { f: 523,  t: 'sine', d: 0.12, v: 0.11, delay: 0.00 },
                { f: 659,  t: 'sine', d: 0.12, v: 0.10, delay: 0.11 },
                { f: 784,  t: 'sine', d: 0.14, v: 0.10, delay: 0.22 },
                { f: 1047, t: 'sine', d: 0.22, v: 0.12, delay: 0.35 },
                { f: 784,  t: 'sine', d: 0.10, v: 0.09, delay: 0.58 },
                { f: 1047, t: 'sine', d: 0.35, v: 0.13, delay: 0.70 },
                { f: 1319, t: 'sine', d: 0.30, v: 0.10, delay: 0.95 },  /* E6 — ذروة */
            ];
            melody.forEach(n => tone(n.f, n.t, n.d, n.v, n.delay, n.delay > 0.5));
            /* هارمونيا خلفية */
            tone(261, 'triangle', 0.80, 0.035, 0.0);
            tone(392, 'triangle', 0.65, 0.030, 0.25);
            tone(523, 'triangle', 0.50, 0.025, 0.50);
            break;
        }

        /* 🛒 شراء — نغمة تأكيد ذهبية */
        case 'purchase': {
            tone(440, 'sine', 0.12, 0.10);
            tone(554, 'sine', 0.14, 0.09, 0.10);
            tone(659, 'sine', 0.20, 0.11, 0.22);
            tone(880, 'sine', 0.18, 0.08, 0.38);
            break;
        }

        /* 🔔 click — نقرة نظيفة */
        case 'click': {
            tone(800, 'sine', 0.055, 0.055);
            tone(1000, 'sine', 0.035, 0.030, 0.03);
            break;
        }

        /* 🎵 فتح قسم/overlay */
        case 'open': {
            toneGlide(350, 520, 'sine', 0.14, 0.08);
            tone(520, 'sine', 0.10, 0.06, 0.14);
            break;
        }

        /* ⏱ tick — نبضة مؤقت */
        case 'tick': {
            tone(1200, 'square', 0.025, 0.018);
            break;
        }

        /* 🔥 combo — نغمة متصاعدة حسب مستوى الـ streak */
        case 'combo3': {
            tone(440, 'sine', 0.09, 0.09);
            tone(554, 'sine', 0.10, 0.08, 0.08);
            break;
        }
        case 'combo5': {
            tone(523, 'sine', 0.09, 0.10);
            tone(659, 'sine', 0.10, 0.09, 0.08);
            tone(784, 'sine', 0.12, 0.09, 0.17);
            break;
        }
        case 'combo10': {
            tone(659, 'sine', 0.10, 0.11);
            tone(784, 'sine', 0.10, 0.10, 0.08);
            tone(1047, 'sine', 0.15, 0.11, 0.18);
            tone(1319, 'sine', 0.14, 0.09, 0.32, true);
            break;
        }

        /* 💰 عملات مكتسبة */
        case 'coin': {
            tone(880, 'sine', 0.07, 0.07);
            tone(1108, 'sine', 0.09, 0.07, 0.07);
            break;
        }

        /* ⚠️ تحذير (عملات قليلة، وقت ينفد) */
        case 'warning': {
            tone(330, 'square', 0.07, 0.06);
            tone(294, 'square', 0.10, 0.06, 0.10);
            break;
        }

        /* 🚪 إغلاق */
        case 'close': {
            toneGlide(520, 350, 'sine', 0.12, 0.07);
            break;
        }

        /* 🛡️ درع */
        case 'shield': {
            tone(392, 'triangle', 0.12, 0.09);
            tone(494, 'triangle', 0.14, 0.08, 0.11);
            tone(587, 'triangle', 0.18, 0.08, 0.24, true);
            break;
        }

        /* 🎯 مهمة مكتملة */
        case 'task': {
            tone(587, 'sine', 0.10, 0.09);
            tone(740, 'sine', 0.12, 0.09, 0.10);
            tone(880, 'sine', 0.16, 0.10, 0.22);
            break;
        }

        /* 🏅 دقة مثالية 100% */
        case 'perfect': {
            tone(523,  'sine', 0.10, 0.10);
            tone(659,  'sine', 0.11, 0.09, 0.08);
            tone(784,  'sine', 0.12, 0.09, 0.18);
            tone(1047, 'sine', 0.20, 0.10, 0.30, true);
            tone(1319, 'sine', 0.18, 0.08, 0.50, true);
            break;
        }

        /* 🌅 مكافأة يومية */
        case 'daily_bonus': {
            tone(440, 'sine', 0.10, 0.09);
            tone(554, 'sine', 0.12, 0.09, 0.12);
            tone(659, 'sine', 0.14, 0.10, 0.26);
            tone(880, 'sine', 0.14, 0.08, 0.40);
            break;
        }
    }

    /* ─── نظام الاهتزاز الذكي ─── */
    _vibrate(type);
}

/* ═══════════════════════════════════════════════════════════════
   ④ نظام الاهتزاز الذكي
═══════════════════════════════════════════════════════════════ */
function _vibrate(type) {
    if (!st || !st.vibrationOn || !navigator.vibrate) return;
    const ms = Math.max(10, Math.min(200, st.vibrationStrength || 30));
    const patterns = {
        click:    [Math.round(ms * 0.4)],
        correct:  [ms],
        wrong:    [ms, 40, ms],
        levelup:  [ms, 30, ms, 30, ms * 2],
        purchase: [ms, 20, ms, 20, ms],
        combo3:   [ms],
        combo5:   [ms, 20, ms],
        combo10:  [ms, 15, ms, 15, ms, 15, ms * 2],
        warning:  [ms, 60, ms],
        shield:   [ms * 2],
        task:     [ms, 25, ms],
        coin:     [Math.round(ms * 0.6)],
    };
    const pattern = patterns[type] || [ms];
    navigator.vibrate(pattern);
}

/* ═══════════════════════════════════════════════════════════════
   ⑤ الموسيقى الخلفية التكيّفية — تتغير مع وتيرة اللعبة
═══════════════════════════════════════════════════════════════ */

/* سلّم موسيقي: C مجور (إيجابي، مبهج) */
const _BG_SCALE = [261.63, 293.66, 329.63, 349.23, 392.00, 440.00, 493.88, 523.25];
/* نمط الأرقيجيو (arpeggio) — ترتيب النغمات */
const _BG_PATTERNS = [
    [0, 2, 4, 7, 4, 2],          /* ارتقاء هادئ */
    [0, 4, 7, 4, 2, 4],          /* تأرجح */
    [0, 2, 4, 2, 0, 4, 7, 4],    /* معقد */
];
let _bgPatternIdx = 0;
let _bgNoteIdx = 0;
let _bgCurrentPattern = _BG_PATTERNS[0];

function bgNote() {
    if (!st || !st.bgOn) return;
    const ctx = gACtx();
    if (!ctx || !_bgGain) return;

    /* تطبيق مستوى الصوت الحالي */
    const vol = (typeof st.bgVolume === 'number' ? st.bgVolume : 60) / 100;
    _bgGain.gain.setValueAtTime(0.022 * vol * (_bgAccelerated ? 1.3 : 1.0), ctx.currentTime);

    const noteIdx = _bgCurrentPattern[_bgNoteIdx % _bgCurrentPattern.length];
    const freq = _BG_SCALE[noteIdx];
    _bgNoteIdx++;

    const o = ctx.createOscillator();
    const g = ctx.createGain();
    o.connect(g);
    g.connect(_bgGain);
    o.type = 'triangle';
    o.frequency.value = freq;

    const noteDur = _bgAccelerated ? 0.30 : 0.55;
    g.gain.setValueAtTime(0, ctx.currentTime);
    g.gain.linearRampToValueAtTime(0.022 * vol, ctx.currentTime + 0.02);
    g.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + noteDur);

    /* نغمة بيس خفية كل 4 نغمات */
    if (_bgNoteIdx % 4 === 0) {
        const bass = ctx.createOscillator();
        const bassG = ctx.createGain();
        bass.connect(bassG);
        bassG.connect(_bgGain);
        bass.type = 'sine';
        bass.frequency.value = freq / 2;
        bassG.gain.setValueAtTime(0, ctx.currentTime);
        bassG.gain.linearRampToValueAtTime(0.018 * vol, ctx.currentTime + 0.03);
        bassG.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + noteDur * 1.2);
        bass.start(ctx.currentTime);
        bass.stop(ctx.currentTime + noteDur * 1.3);
    }

    o.start(ctx.currentTime);
    o.stop(ctx.currentTime + noteDur + 0.01);
}

/* بدء الموسيقى الخلفية */
function startBg() {
    if (!bgInt) {
        const interval = _bgAccelerated ? 280 : 480;
        bgInt = setInterval(bgNote, interval);
    }
}

/* إيقاف الموسيقى الخلفية */
function stopBg() {
    clearInterval(bgInt);
    bgInt = null;
}

/* تسريع الخلفية عند ضغط المؤقت (< 10 ثوان) */
function setBgAccelerated(isAccel) {
    if (_bgAccelerated === isAccel) return;
    _bgAccelerated = isAccel;
    /* إعادة تشغيل بالـ BPM الجديد */
    if (bgInt) {
        stopBg();
        startBg();
    }
    /* تغيير النمط عند التسريع */
    if (isAccel) {
        _bgCurrentPattern = _BG_PATTERNS[2]; /* النمط الأكثر إثارة */
        playSound('warning');
    } else {
        _bgCurrentPattern = _BG_PATTERNS[0];
    }
}

/* تغيير النمط الموسيقي حسب وضع اللعبة */
function setBgMoodForMode(mode) {
    switch (mode) {
        case 'speed':    _bgCurrentPattern = _BG_PATTERNS[1]; break;
        case 'frenzy':   _bgCurrentPattern = _BG_PATTERNS[2]; break;
        case 'survival': _bgCurrentPattern = _BG_PATTERNS[0]; break;
        default:         _bgCurrentPattern = _BG_PATTERNS[0]; break;
    }
    _bgNoteIdx = 0;
}

/* ═══════════════════════════════════════════════════════════════
   ⑥ دوال تبديل الصوت
═══════════════════════════════════════════════════════════════ */
function toggleBgMusic() {
    st.bgOn = !st.bgOn;
    ['bgMusicStatus', 'gbgMusicStatus'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = st.bgOn ? 'مفعّلة' : 'مطفأة';
    });
    st.bgOn ? startBg() : stopBg();
    playSound('click');
    saveSt();
}

function toggleSound() {
    st.soundOn = !st.soundOn;
    ['soundStatus', 'gsoundStatus'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = st.soundOn ? 'مفعّل' : 'مطفأ';
    });
    saveSt();
    if (st.soundOn) playSound('click');
}

/* ═══════════════════════════════════════════════════════════════
   ⑦ مزامنة صوت المؤقت مع checkAnswer في game.js
   يستدعى تلقائياً عند كل إجابة لتحديث BPM الخلفية
═══════════════════════════════════════════════════════════════ */
function _syncBgWithTimer() {
    if (!G || !G.hasTimer || !G.maxTime) return;
    const pct = G.timeLeft / G.maxTime;
    if (pct <= 0.2 && !_bgAccelerated) {
        setBgAccelerated(true);
    } else if (pct > 0.2 && _bgAccelerated) {
        setBgAccelerated(false);
    }
}

/* hook على setInterval الخاص باللعبة — يُستدعى من checkAnswer */
window._onGameTimerTick = function() {
    _syncBgWithTimer();
    if (G.hasTimer && G.timeLeft <= 5 && G.timeLeft > 0) {
        playSound('tick');
    }
};

/* ═══════════════════════════════════════════════════════════════
   ⑧ playSound للـ combo — يختار المؤثر المناسب لمستوى التتابع
═══════════════════════════════════════════════════════════════ */
/* تُستدعى من checkAnswer بدلاً من showComboEffect */
function playComboSound(streak) {
    if (streak >= 10) playSound('combo10');
    else if (streak >= 5) playSound('combo5');
    else if (streak >= 3) playSound('combo3');
}

/* ═══════════════════════════════════════════════════════════════
   ⑨ استعادة AudioContext بعد سبات المتصفح
═══════════════════════════════════════════════════════════════ */
document.addEventListener('visibilitychange', function() {
    if (!document.hidden && aCtx && aCtx.state === 'suspended') {
        aCtx.resume().catch(() => {});
    }
    if (document.hidden) {
        /* إيقاف مؤقت للخلفية عند إخفاء الصفحة */
        stopBg();
    } else if (st && st.bgOn) {
        setTimeout(startBg, 300);
    }
});

/* تشغيل AudioContext عند أول لمسة/نقر */
document.addEventListener('touchstart', function() {
    gACtx();
}, { once: true, passive: true });

document.addEventListener('click', function() {
    gACtx();
    if (aCtx && aCtx.state === 'suspended') aCtx.resume().catch(() => {});
}, { once: true });

