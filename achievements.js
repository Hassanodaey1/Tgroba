/* ═══════════════════════════════════════════════════════════════
   HO Math v11 — ACHIEVEMENTS & DAILY TASKS (Fixed & Unified)
   © 2026 Hassan Odaey
═══════════════════════════════════════════════════════════════ */
...
function checkAchievements() {
    ...
    ACHIEVEMENTS_DEF.forEach(a => {
        if (!st.achievementsUnlocked.includes(a.id) && a.check()) {
            ...
            if (a.reward > 0) {
                earnCoins(a.reward, 'achievement');
            }
...
/* ═══════════════════════════════════════════════════
   عرض الإنجازات في الواجهة
════════════════════════════════════════════════════ */
var _showAllAchievements = false;
...
function renderAchievements() {
    ...
    if (showBtn) {
        ...
        showBtn.textContent = _showAllAchievements ? '🔼 عرض أقل' : '📋 عرض جميع الإنجازات الـ ' + total;
    }
...
/* ─── دالة موحدة لإعادة تعيين اليوم (تستدعى من checkDailyReset في state.js) ─── */
...
function updTask(type, amount = 1) {
    // نضمن وجود المهام اليومية
    if (!st.dailyTasks || st.dailyTasks.length === 0 || st.dailyDate !== todayStr()) {
        resetDailyData();
    }
...
/* ═══════════════════════════════════════════════════
   مؤقت الجلسة ووقت اللعب
════════════════════════════════════════════════════ */
var sessionStart = Date.now();

function getSessionSecs() {
    if (st.sessionDate !== todayStr()) {
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        st.sessionTimeSecs = getSessionSecs();
        st.sessionDate = todayStr();
        saveSt();
...
/* ═══════════════════════════════════════════════════
   عدادات اليومية والأسبوعية والشهرية
════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════
   بيانات القوانين (لشاشة القوانين)
════════════════════════════════════════════════════ */
const LAWS_DATA = [
