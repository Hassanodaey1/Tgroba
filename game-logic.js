...
function sanitizeState(s) {
    ...
    if (typeof s.competitiveBest !== 'number') s.competitiveBest = 0;
    return s;
}
...
const LAWS_DATA = [
    ...
    {
        cat: 'الدوال الأسية واللوغاريتمية الطبيعية',
        laws: [
...
function getDynamicDifficulty() {
    const corr = G.correct;
    if (corr < 5) return 'easy';
    if (corr < 15) return 'medium';
    if (corr < 30) return 'hard';
    return 'genius';
}
...
function genQ(op, diff, customTable = null) {
    ...
    if (G.mode === 'competitive') {
        ch = ['add','sub','mul','div'][rnd(0,3)];
        actualDiff = getDynamicDifficulty();
        const cr = ranges[actualDiff] || ranges.easy;
        const builder = questionBuilders[ch] || questionBuilders['add'];
        let qObj = builder(cr);
        let wrongs = new Set();
        let ans = qObj.answer;
...
function startGameWith(mode, op, customTable = null, forceTimer = false) {
    ...
    else if (mode === 'competitive') { G.totalQ = 9999; hasTimer = false; lives = 0; }
    G.livesLeft = lives; G.hasTimer = hasTimer;
    G.helpersUsed = { skip: false, remove: false };
    const titles = { classic:'🧮 كلاسيك', speed:'⚡ سرعة 60ث', survival:'🔥 التحمّل', frenzy:'💥 اندفاع', daily:'🌟 تحدي اليوم', marathon:'🏃 ماراثون', competitive:'🏆 تحدي المنافسة' };
    ...
    const endBtn = document.getElementById('competitiveEndBtn');
    if (endBtn) { endBtn.style.display = (mode==='competitive') ? 'block' : 'none'; }
    if (hasTimer) {
...
function loadQuestion() {
    ...
    do {
        ...
        const isEndless = G.mode==='speed' || G.mode==='survival' || G.mode==='frenzy' || G.mode==='competitive';
    ...
    document.getElementById('questionNumber').textContent = G.isTraining ? `🎓 تدريب - ${G.correct+1}` :
        ...
        G.mode==='competitive' ? `🏆 الإجابة ${G.correct+1} (صعوبة ${getDynamicDifficulty()})` :
        `السؤال ${G.currentQ} من ${G.totalQ}`;
    document.getElementById('questionText').textContent = `${q.text} = ?`;
    document.getElementById('questionHint').textContent = q.hint || 'ما هو الجواب؟';
    document.getElementById('statQ').textContent = (G.isTraining || G.mode==='speed' || G.mode==='survival' || G.mode==='frenzy' || G.mode==='competitive') ? G.correct : `${G.currentQ}/${G.totalQ}`;
...
function checkAnswer(btn) {
    ...
    if (val === G.correctAnswer) {
        ...
        } else if (G.mode === 'competitive') {
            showFeedback('❌'); playSound('wrong'); showExplanation();
            // في الوضع التنافسي، لا نوقف اللعبة عند الخطأ، فقط نستمر
        } else {
            if (G.livesLeft>0) {
    ...
    updateGameCoinsDisplay();
    saveSt();
    const delay = (G.mode === 'competitive') ? 100 : 350;
    setTimeout(() => {
        if (G.ended) return;
        if (!G.isTraining && G.mode!=='speed' && G.mode!=='survival' && G.mode!=='frenzy' && G.mode!=='competitive' && G.currentQ>=G.totalQ) endGame();
        else loadQuestion();
    }, delay);
}
...
function endGame() {
    ...
    if (G.mode==='competitive') {
        if (G.correct > (st.competitiveBest || 0)) {
            st.competitiveBest = G.correct;
            if (G.correct > st.bestScore) {
                st.bestScore = G.correct;
                syncLeaderboard(st.name, st.avatar, st.level, G.correct);
            }
        }
        ...
        document.getElementById('resultsEmoji').textContent = acc>=90?'🏆':acc>=70?'⭐':'💪';
        document.getElementById('resultsTitle').textContent = 'انتهى التحدي!';
        ...
        document.getElementById('resultsXP').textContent = `+${xpGained} XP • +0 💰`;
    ...
    updateUI();
    checkAchievements();
    // مزامنة النتيجة إلى لوحة المتصدرين إذا كانت أفضل
    if (G.score > (st.bestScore || 0)) {
        st.bestScore = G.score;
        syncLeaderboard(st.name, st.avatar, st.level, G.score);
    }
...
function endCompetitiveChallenge() {
    if (G.mode !== 'competitive' || G.ended) return;
    G.ended = true; clearGameTimer();
    if (G.correct > (st.competitiveBest || 0)) {
        st.competitiveBest = G.correct;
        if (G.correct > st.bestScore) {
