...
function genQ(op, diff, customTable = null) {
    ...
    if (op === 'advanced') {
        ...
        if (ch === 'power') {
            ...
            text = `√${a}`; hint = 'ما الجذر التربيعي؟'; explanation = `√${a} = ${ans}`;
            ...
            let c = rnd(2, 8); let a2 = rnd(1, c - 1); let b2 = rnd(1, c - 1); ans = a2 + b2;
            text = `${a2}/${c} + ${b2}/${c}`; hint = 'اجمع الكسور (المقامات متساوية)'; explanation = `${a2}/${c} + ${b2}/${c} = ${ans}/${c}`;
        } else if (ch === 'percent') {
            let pct = [10, 20, 25, 50][rnd(0, 3)]; a = rnd(1, 20) * 10; ans = Math.round(a * pct / 100);
            text = `${pct}% من ${a}`; hint = 'احسب النسبة المئوية'; explanation = `${a} × ${pct}% = ${ans}`;
        } else if (ch === 'sequence') {
            a = rnd(1, 10); b = rnd(2, 8); text = `${a}, ${a + b}, ${a + 2 * b}, ?`; ans = a + 3 * b;
            hint = 'ما الرقم التالي في المتتالية؟'; explanation = `الفرق = ${b}، الرقم التالي = ${ans}`;
        } else if (ch === 'log_simple') {
            a = rnd(2, 4); ans = a; text = `log₁₀(10^${a})`; hint = `ما قيمة اللوغاريتم؟`; explanation = `log₁₀(10^${a}) = ${a}`;
        } else if (ch === 'trig_simple') {
            let degs = [0, 30, 45, 60, 90]; let deg = degs[rnd(0, degs.length - 1)]; let val = Math.round(Math.sin(deg * Math.PI / 180) * 10) / 10; ans = val;
            text = `جا(${deg}°)`; hint = `ما قيمة جيب الزاوية ${deg}°؟`; explanation = `جا(${deg}°) = ${val}`;
        } else if (ch === 'area_circle') {
            let r = rnd(3, 8); ans = Math.round(Math.PI * r * r); text = `مساحة دائرة نصف قطرها ${r}`; hint = 'استخدم π≈3.14'; explanation = `المساحة = π × ${r}² = ${ans}`;
        } else {
            a = rnd(2, 8); ans = rnd(2, 12); b = 2 * ans + a; text = `2س + ${a} = ${b}`; hint = 'ما قيمة س؟'; explanation = `س = (${b} - ${a})/2 = ${ans}`;
        }
    ...
    if (ch === 'add') {
        ...
        a = rnd(r.small[0], r.small[1]); b = rnd(r.small[0], r.small[1]); if (a < b) [a, b] = [b, a]; ans = a - b;
        ...
        const pcts = [10, 20, 25, 50, 75]; const pct = pcts[rnd(0, pcts.length - 1)]; a = rnd(1, 20) * 10; ans = Math.round(a * pct / 100);
        text = `${pct}% من ${a}`; hint = 'احسب النسبة المئوية'; explanation = `${a} × ${pct}% = ${ans}`;
    } else if (ch === 'power') {
        a = rnd(2, 10); b = rnd(2, 3); ans = Math.pow(a, b); text = `${a}^${b}`; hint = `ما قيمة ${a} مرفوعاً للأس ${b}؟`; explanation = `${a}^${b} = ${ans}`;
    } else if (ch === 'sqrt') {
        const sq = [4, 9, 16, 25, 36, 49, 64, 81, 100, 121, 144]; a = sq[rnd(0, sq.length - 1)]; ans = Math.round(Math.sqrt(a));
        text = `√${a}`; hint = 'ما الجذر التربيعي؟'; explanation = `√${a} = ${ans} لأن ${ans}² = ${a}`;
    } else if (ch === 'equation_simple') {
        a = rnd(5, 30); b = rnd(a + 1, a + 50); ans = b - a; text = `س + ${a} = ${b}`; hint = 'ما قيمة س؟'; explanation = `س = ${b} - ${a} = ${ans}`;
    } else if (ch === 'sequence') {
        a = rnd(1, 15); b = rnd(2, 10); text = `${a}, ${a + b}, ${a + 2 * b}, ?`; ans = a + 3 * b;
        hint = 'ما الرقم التالي في المتتالية؟'; explanation = `الفرق = ${b}، الرقم التالي = ${ans}`;
    } else if (ch === 'fraction_simple') {
        let d = rnd(2, 6); let n1 = rnd(1, d - 1); let n2 = rnd(1, d - 1); ans = n1 + n2;
        text = `${n1}/${d} + ${n2}/${d}`; hint = 'اجمع الكسور ذات المقام المشترك'; explanation = `الناتج = ${ans}/${d}`;
    } else if (ch === 'word_add') {
        let x = rnd(10, 50); let y = rnd(5, 30); ans = x + y;
        text = `لدى أحمد ${x} تفاحة واشترى ${y} تفاحة أخرى. كم تفاحة لديه الآن؟`; hint = 'جمع بسيط'; explanation = `${x}+${y}=${ans}`;
    } else if (ch === 'word_mul') {
        let p = rnd(2, 8); let q = rnd(2, 12); ans = p * q;
        text = `إذا كان ثمن القلم الواحد ${p} ديناراً، فكم ثمن ${q} أقلام؟`; hint = 'ضرب'; explanation = `${p}×${q}=${ans}`;
    } else if (ch === 'log_simple') {
        let exp = rnd(1, 3); ans = exp; text = `log₁₀(10^${exp})`; hint = 'ما قيمة اللوغاريتم؟'; explanation = `log₁₀(10^${exp}) = ${exp}`;
    } else {
        ch = 'add'; a = rnd(1, 20); b = rnd(1, 20); ans = a + b; text = `${a} + ${b}`; hint = 'ما مجموع العددين؟'; explanation = `${a} + ${b} = ${ans}`;
    }
    const wr = new Set();
