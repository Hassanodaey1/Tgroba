...
function startTrainingMode(op = 'mix') {
    ...
    G.timer = setInterval(() => {
        ...
        else { G.timeLeft--; const pct = (G.timeLeft / G.maxTime) * 100; document.getElementById('timerBar').style.width = pct + '%'; document.getElementById('bigTimer').textContent = G.timeLeft; if (G.timeLeft <= 10) { document.getElementById('timerBar').classList.add('danger'); document.getElementById('bigTimer').classList.add('danger'); } if (G.timeLeft > 0 && G.timeLeft <= 6) playSound('tick'); }
    }, 1000);
...
function selectTrainingTime() {
    showConfirm('اختر وقت التدريب', '30 ثانية أو 60 ثانية؟', '30 ثانية', '60 ثانية', (is30) => {
        if (is30) { G.maxTime = 30; G.timeLeft = 30; } else { G.maxTime = 60; G.timeLeft = 60; }
        document.getElementById('bigTimer').textContent = G.timeLeft;
        document.getElementById('timerBar').style.width = '100%';
        if (G.timer) clearInterval(G.timer);
        G.timer = setInterval(() => {
...
function startCompetitionGame() {
    startGameWith('competition', 'mix', null, true);
}
...
function startGameWith(mode, op, customTable = null, forceTimer = false) {
    ...
    G.hasTimer = hasTimer;
    G.helpersUsed = { skip: false, remove: false };
    const titles = { classic: '🧮 كلاسيك', speed: '⚡ سرعة 60ث', survival: '🔥 التحمّل', frenzy: '💥 اندفاع', daily: '🌟 تحدي اليوم', competition: '⚔️ المنافسة' };
    ...
    if (hasTimer) {
        ...
        G.timer = setInterval(() => {
            if (G.ended) { clearInterval(G.timer); G.timer = null; return; }
            if (G.timeLeft <= 0) { clearInterval(G.timer); G.timer = null; if (!G.ended) endGame(); }
            else { G.timeLeft--; const pct = (G.timeLeft / G.maxTime) * 100; document.getElementById('timerBar').style.width = pct + '%'; bt.textContent = G.timeLeft; if (G.timeLeft <= 10) { document.getElementById('timerBar').classList.add('danger'); bt.classList.add('danger'); } if (G.timeLeft > 0 && G.timeLeft <= 5) playSound('tick'); }
        }, 1000);
    } else { document.getElementById('bigTimerWrap').style.display = 'none'; }
    updateGameCoinsDisplay();
    document.getElementById('resultsOverlay').classList.remove('active');
    document.getElementById('gameOverlay').classList.add('active');
    loadQuestion();
...
function useHelper(type) {
    ...
    if (type === 'skip') {
        ...
        document.getElementById('helperSkip').classList.add('used'); saveSt(); updateUI(); updateGameCoinsDisplay();
        if (G.hasTimer && G.maxTime > 0) { G.timeLeft = Math.max(0, G.timeLeft - 4); const pct = (G.timeLeft / G.maxTime) * 100; document.getElementById('timerBar').style.width = pct + '%'; const bt = document.getElementById('bigTimer'); if (bt) bt.textContent = G.timeLeft; if (G.timeLeft <= 0) { clearGameTimer(); endGame(); return; } }
        ...
        document.getElementById('helperRemove').classList.add('used'); saveSt(); updateUI(); updateGameCoinsDisplay();
        ...
        if (wrongs.length > 0) { const rem = wrongs[Math.floor(Math.random() * wrongs.length)]; rem.style.opacity = '0.15'; rem.style.pointerEvents = 'none'; showFeedback('🗑️ حُذفت إجابة خاطئة'); } else showFeedback('⚠️ لا توجد إجابات خاطئة للحذف');
    } else if (type === 'heart') {
...
function loadQuestion() {
        ...
        if (G.isTraining) {
            ...
            if (G.op === 'table' && G.customTable) { q = genQ('table', st.difficulty, G.customTable); }
                ...
                if (G.op === 'advanced') q = genQ('advanced', st.difficulty);
                    ...
                    var cDiff;
                    if (G.correct <= 5) cDiff = 'easy';
                    else if (G.correct <= 15) cDiff = 'medium';
                    else if (G.correct <= 30) cDiff = 'hard';
                    else cDiff = 'genius';
                    q = genQ('mix', cDiff);
                } else {
    ...
    const isEndlessMode = G.mode === 'speed' || G.mode === 'survival' || G.mode === 'frenzy' || G.mode === 'competition';
    if (!G.isTraining && !isEndlessMode) { G.askedQuestions.push(q.text + '|' + q.answer); }
    ...
    if (G.isTraining) { document.getElementById('questionNumber').textContent = `🎓 تدريب - ${G.correct+1}`; }
        ...
        document.getElementById('questionNumber').textContent =
            ...
            G.mode === 'competition' ? `⚔️ السؤال ${G.correct+1} • نقاط: ${G.score}` :
            `السؤال ${G.currentQ} من ${G.totalQ}`;
    }
    document.getElementById('questionText').textContent = `${q.text} = ?`;
    document.getElementById('questionHint').textContent = q.hint || 'ما هو الجواب؟';
    document.getElementById('statQ').textContent = (G.isTraining || G.mode === 'speed' || G.mode === 'survival' || G.mode === 'frenzy' || G.mode === 'competition') ? G.correct : `${G.currentQ}/${G.totalQ}`;
    renderVisualAid(q);
