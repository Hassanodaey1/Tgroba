...
/* ✅ FIX-2.3: دالة تبسيط الكسر — تُعرض الكسور بشكل رياضي صحيح */
        ...
        function genQ(op, diff, customTable = null) {
            ...
            if (op === 'advanced') {
                ...
                if (ch === 'power') {
                    ...
                    /* كسور بمقامات مختلفة — إصلاح: d2 يُعاد توليده حتى يختلف عن d1 */
                    let d1=rnd(2,5); let d2=rnd(2,5);
                    let _safetyFD = 0;
                    while(d2===d1 && _safetyFD<20){ d2=rnd(2,5); _safetyFD++; }
                    if(d2===d1) d2 = d1===5 ? 2 : d1+1; /* ضمان اختلاف المقامين */
            ...
            if (ch === 'add') { a = rnd(r.small[0], r.small[1]);
                ...
                explanation = `${n1}/${d} + ${n2}/${d} = ${_simplified}`; } else if (ch === 'word_add') {
                let x = rnd(10, 50);
                let y = rnd(5, 30);
                ans = x + y;
                const _waNames = ['أحمد','سارة','خالد','ليلى','محمد','هند','عمر','نورة'];
                const _wan = _waNames[rnd(0,_waNames.length-1)];
                const _waCtxs = [
                    { t:`لدى ${_wan} ${x} تفاحة واشترى ${y} أخرى. كم تفاحة لديه الآن؟`,       e:`${x}+${y}=${ans}` },
                    { t:`جمع ${_wan} ${x} طابعاً و${y} طابعاً آخر. كم طابعاً معه؟`,            e:`${x}+${y}=${ans}` },
                    { t:`في الحديقة ${x} وردة حمراء و${y} وردة صفراء. المجموع = ؟`,            e:`${x}+${y}=${ans}` },
                    { t:`ركب ${_wan} ${x} كم صباحاً و${y} كم مساءً. المسافة الكلية = ؟`,      e:`${x}+${y}=${ans} كم` }
                ];
                const _wa = _waCtxs[rnd(0,_waCtxs.length-1)];
                text = _wa.t; hint = 'اجمع العددين'; explanation = _wa.e;
            } else if (ch === 'word_mul') {
                let p = rnd(2, 8);
                let q = rnd(2, 12);
                ans = p * q;
                const _wmCtxs = [
                    { t:`ثمن القلم ${p} دينار. ما ثمن ${q} قلم؟`,                     e:`${q}×${p}=${ans} دينار` },
                    { t:`سيارة تقطع ${p} كم في الساعة. في ${q} ساعات تقطع = ؟`,       e:`${p}×${q}=${ans} كم` },
                    { t:`${p} صناديق في كل صندوق ${q} تفاحة. الإجمالي = ؟`,           e:`${p}×${q}=${ans}` },
                    { t:`مزرعة بها ${p} صفوف وكل صف ${q} شجرة. عدد الأشجار = ؟`,    e:`${p}×${q}=${ans} شجرة` }
                ];
                const _wm = _wmCtxs[rnd(0,_wmCtxs.length-1)];
                text = _wm.t; hint = 'اضرب العددين'; explanation = _wm.e; } else if (ch === 'log_simple') { let exp = rnd(1, 3);
                ans = exp;
                text = `log₁₀(10^${exp})`;
        ...
        function useHelper(type) {
            ...
            /* ─── تحقق من المخزون أولاً (5.3) ─── */
            const fromInv = typeof useHelperFromInventory === 'function'
                ? useHelperFromInventory(type)
                : false;
