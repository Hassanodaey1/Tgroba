/* ═══════════════════════════════════════════════════════════════
   HO Math v11 — UI HANDLER (Profile, Emoji Shop, Themes)
   © 2026 Hassan Odaey
═══════════════════════════════════════════════════════════════ */

/* ═══════════════════════════════════════════════════
   إعدادات الصوت والاهتزاز (موجودة أيضاً في audio_effects.js ولكن نضمنها هنا)
════════════════════════════════════════════════════ */
function openGameSettings() { openGameSettingsAndPause(); }

/* ═══════════════════════════════════════════════════
   متجر الإيموجي (الشخصيات)
════════════════════════════════════════════════════ */
const EMOJI_CATALOG = [
...
function buyOrSelectEmoji(emoji, price) {
    ...
    showConfirm('شراء رمز', 'هل تريد شراء هذا الرمز بـ ' + price + ' عملة؟', 'نعم', 'إلغاء', ok => {
        if (ok) {
            if (!spendCoins(price, 'emoji_shop')) { showFeedback('💸 لا يكفي!'); return; }
...
function renderProfileDailyTasks() {
    ...
    if(total===0){ container.innerHTML='<div style="font-size:0.75em;color:var(--text2);text-align:center;padding:10px;">لا توجد مهام بعد</div>'; return; }
...
function updateCompetitionNavStyle(){
    ...
    style.textContent = `#nav-leaderboard.active .nav-item-icon, #nav-leaderboard.active .nav-item-label { color: ${gold} !important; } #nav-leaderboard.active .nav-active-dot { background: ${gold} !important; box-shadow: 0 0 8px ${gold}88; } #nav-leaderboard .nav-item-icon { background: linear-gradient(135deg, ${gold}22, ${gold}11); border-radius: 10px; padding: 2px 4px; }`;
...
function confirmResetStats(){
    showConfirm('تصفير الإحصائيات', 'هل أنت متأكد من تصفير جميع الإحصائيات؟\nلن يتأثر ملفك الشخصي أو عملاتك أو الرموز المشتراة.', 'تأكيد', 'إلغاء', ok=>{
        ...
        st.catCounter={correct:0,total:0}; st.catChallenges={games:0}; st.history=[]; st.xp=0; st.level=1; st.xpToNext=1000;
        for(let k in st.stats) st.stats[k]={att:0,cor:0,first:0,stars:0,max:0};
        st.achievementsUnlocked=[]; st.achievementRewardClaimed=false; st.dailyStreak=0; st.lastDailyDate=null; st.dailyShieldUsed=false;
        saveSt(); updateUI(); playSound('levelup'); showFeedback('📊 تم التصفير!');
    });
}
/* ═══════════════════════════════════════════════════
   تحديث الواجهة الرئيسية (updateUI)
════════════════════════════════════════════════════ */
function updateUI() {
...
/* ═══════════════════════════════════════════════════
   الصورة الشخصية (رفع وإدارة)
════════════════════════════════════════════════════ */
function triggerPhotoUpload() {
    showConfirm('📷 الصورة الشخصية', 'اختر مصدر الصورة', '📷 كاميرا', '🖼️ معرض الصور', function(useCamera) {
...
function applyProfilePhoto() {
    ...
    if (st.profilePhoto) {
        if (bigEl) { bigEl.style.backgroundImage = 'url("' + st.profilePhoto + '")'; bigEl.style.backgroundSize = 'cover'; bigEl.style.backgroundPosition = 'center'; bigEl.textContent = ''; }
        if (headerEl) { headerEl.style.backgroundImage = 'url("' + st.profilePhoto + '")'; headerEl.style.backgroundSize = 'cover'; headerEl.style.backgroundPosition = 'center'; headerEl.textContent = ''; }
...
/* ═══════════════════════════════════════════════════
   لائحة الصدارة (Leaderboard) الموحدة
════════════════════════════════════════════════════ */
...
function loadCombinedLeaderboard() {
    ...
    try {
        if (_activeLbTab === 'challenge') {
            ...
            }).catch(() => { container.innerHTML = '<div style="text-align:center;padding:16px;color:var(--text2);">⚠️ فشل التحميل</div>'; });
        } else {
    ...
    } catch(e) { container.innerHTML = '<div style="text-align:center;padding:16px;color:var(--text2);">⚠️ خطأ في الاتصال</div>'; }
}
...
/* مزامنة مع Firebase (نسخة محسنة مع التحقق من database) */
function syncWithLeaderboard() {
    if (!database) return;
    try {
        const playerKey = _getPlayerKey();
        database.ref('leaderboard/' + playerKey).set({
            name: st.name, avatar: st.avatar, level: st.level, bestScore: st.bestScore, totalXp: st.xp, lastUpdated: Date.now()
        }).catch(()=>{});
        database.ref('challenge_leaderboard/' + playerKey).set({
            name: st.name, avatar: st.avatar, level: st.level, challengeScore: st.challengeBestScore || 0, lastUpdated: Date.now()
        }).catch(()=>{});
        syncWeeklyScore();
    } catch(e) {}
}
