...
function openAdvancedGame() { if (st.level >= 7) startGameWith('classic', 'advanced', null, true); else showFeedback('🔒 هذا القسم مقفول حتى المستوى 7'); }
function openLawsGame() { if (st.level >= 10) startGameWith('classic', 'laws', null, true); else showFeedback('🔒 هذا القسم مقفول حتى المستوى 10'); }
...
function openOpSheet(cat, forceTimerForCounter = false) {
    ...
    if (cat === 'counter') {
        ...
        grid.innerHTML = `
            ...
            <div class="mode-card" onclick="startGameWith('competition','mix', null, true)"><span class="mode-card-icon">⚔️</span><div class="mode-card-name">المنافسة</div><div class="mode-card-desc">60 ثانية • صعوبة متصاعدة</div></div>
